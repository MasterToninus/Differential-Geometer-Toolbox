...
\begin{document}
  \begin{landscape}
	...
	\begin{minipage}[c][.55\textheight]{0.46 \linewidth}
		...
		\fbox{
		  \parbox{\textwidth}{
			A \emph{(graded-)commutatative ($I$-)graded algebra} $(V, \wedge)$ is an algebra over the ring $R$ such that :
			\begin{displaymath}
				V = \bigoplus_{i \in I} V_i
			\end{displaymath}
			and
			\begin{displaymath}
				v_{(i)} \wedge w_{(j)} = (-)^{i j} w_{(j)} \wedge v_{(i)} \qquad \forall v_{(i)} \in V_{i}, \quad w_{(j)} \in V_j
			\end{displaymath}
		  }
		}
		\vfill			
