...
\begin{document}
  \begin{landscape}
    \thispagestyle{empty}
    \noindent
    \paragraph{ABSOLUTE DIFFERENTIAL CALCULUS {\small(Calculus of tensors)}}
    	\mbox{}\\
    ...
    \begin{tabularx}{\linewidth}{|c|X|c|c|c|c|}
      ...
      \hline
      	$\ExtD$ & $\left(\dfrac{\partial f }{\partial x^\nu} \right) \: \ExtD x^\nu$ & 0 & n/a & n/a  & $\left( \ExtD \omega \right) \wedge \beta + (-)^k \omega \wedge \left( \ExtD\beta \right) $\\
%
		  ...
		  \parbox{\textwidth}{
		  	...
		  	\begin{align*}
		  		\nabla: \mathfrak{X}(M) \times \mathfrak{X}(M) &\longrightarrow \mathfrak{X}(M) \\
		  				  												(X,Y) &\longmapsto \nabla_X Y		  	
		  	\end{align*}
			such that: \hfill {\small ($\forall a \in \mathbb{R}, f \in C^\infty(M)$)}% $\forall X,Y,Z \in \mathfrak{X}(M)$, $\forall a \in \Real$, $\forall f \in C^\infty(M)$:
		  	\begin{align}
		  		\nabla_{(X+ a Z)} Y &= \nabla_X Y + a \nabla_Z Y \\
		  		\nabla_{X} \left( Y+ a Z \right) &= \nabla_X Y + a \nabla_X Z \\
		  		\nabla_{f\, X} Y &= f \, \nabla_X Y \\
		  		\nabla_{X}\left( f \, Y\right) &= \left(X( f ) \right)\, Y + f \, \left(\nabla_X Y \right)		
		  	\end{align}
		  }
		}
		\vfill
		...
		\fbox{
		  \parbox{\textwidth}{
		  	...
		  	\begin{displaymath}
		  		...
		  		\nabla_X\braket{ \omega | Y} - 
		  		\braket{ \omega | \nabla_X Y} 
		  	\end{displaymath}
		  }
		}	
	\end{minipage}
		\footnote[4]{$g: N \to M$,  $(y^A) \mapsto (x^\mu) $}
		\footnote[3]{Caveat: pull-back is well defined on arbitrary tensors when  $g$ is a diffeomorphism. Otherwise is only defined on purely covariant tensors.}
  \end{landscape}
\end{document}